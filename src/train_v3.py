...
from analysis_v3 import calculate_sii
...
try:
    for i in range(args.max_gen):       
        ...
        print(comp_ps[-1])
        # ====================== SII Analysis ==============================
        # Analyze every generation (or every N if slow)
        if i % 1 == 0: 
            sii_metrics = calculate_sii(speaker, listener)
            sii_metrics['generation'] = i
            sii_history.append(sii_metrics)
            print(f"Gen {i}: SII_Color={sii_metrics['sii_color']:.4f}, SII_Shape={sii_metrics['sii_shape']:.4f}")
    ...
    np.save(save_path+'sii_history.npy', sii_history)
    if 'max_msg_all' in locals():
